...
DELIMITER // 
	CREATE PROCEDURE sp_XepLoaiSinhVienTheoDiemTB (
		IN p_Ma_sv VARCHAR(10),
        OUT p_ThongBao VARCHAR(100)
    )
    BEGIN
		DECLARE p_sv_score DECIMAL(4,2);
        DECLARE p_exists_id INT;
		SELECT COUNT(*) INTO p_exists_id FROM SinhVien
        WHERE MaSV = p_Ma_sv;
		IF p_exists_id = 0 THEN
			SET p_ThongBao = 'Không tồn tại mã sinh viên';
        ELSE
			SELECT AVG(Diem) INTO p_sv_score FROM Diem
			WHERE MaSV = p_Ma_sv;
			IF p_sv_score IS NULL THEN
				SET p_ThongBao = 'Chưa có điểm';

			ELSEIF p_sv_score > 8.5 THEN
				SET p_ThongBao = 'Xuất sắc - Học bổng cao';
			ELSEIF p_sv_score >= 8 THEN
...
SET @kq = '';
CALL sp_XepLoaiSinhVienTheoDiemTB('SV023', @kq);
SELECT @kq;
...
SET @kq = '';
CALL sp_DangKyMonHoc('SV003', 'MH001', @kq);
SELECT @kq AS KetQua; 
...
DELIMITER // 
    ...
    BEGIN
		...
		ELSE
			SELECT COUNT(*) INTO v_count FROM Diem
            WHERE MaSV = p_MaSV AND MaMH = p_MaMH;
...
SET @kq = '';
CALL sp_CapNhatDiem('SV001', 'MH001', 9, @kq);
SELECT @kq;

SELECT * FROM Diem WHERE MaSV = 'SV001' AND MaMH = 'MH001';